...
function runProfileGateTests() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        assert(!(0, profileService_1.isUserProfileComplete)(incompleteHarvester), 'Incomplete Harvester (missing email & phone) is rejected');
        ...
        assert((0, profileService_1.isUserProfileComplete)(completeHarvester), 'Complete Harvester (name, email, phone) is accepted');
        ...
        assert(!(0, profileService_1.isUserProfileComplete)(incompleteProcessor), 'Incomplete Processor (missing organization, facility location, license) is rejected');
        ...
        assert((0, profileService_1.isUserProfileComplete)(completeProcessor), 'Complete Processor (name, phone, email, organization, location, license) is accepted');
        ...
        assert(!(0, profileService_1.isUserProfileComplete)(incompleteLab), 'Incomplete Lab (missing lab name, lab address, accreditation) is rejected');
        const completeLab = {
        ...
        assert((0, profileService_1.isUserProfileComplete)(completeLab), 'Complete Lab (authorized name, phone, email, lab name, address, accreditation) is accepted');
        ...
        assert(!(0, profileService_1.isUserProfileComplete)(incompletePackager), 'Incomplete Packager (missing packaging unit, facility location, FSSAI license) is rejected');
        ...
        assert((0, profileService_1.isUserProfileComplete)(completePackager), 'Complete Packager (authorized name, phone, email, company, location, license) is accepted');
        // 5. Test Database Operations & Workflow Restriction with Prisma
        console.log('\n── 5. Testing Database User Lifecycle & Profile State ──');
        const testIncompleteUser = yield prisma.user.upsert({
            where: {
                email_role: {
        ...
        assert(!(0, profileService_1.isUserProfileComplete)(testIncompleteUser), 'Database loaded incomplete user is evaluated as incomplete');
        // Update user with complete profile data
        const testCompletedUser = yield prisma.user.update({
        ...
        assert((0, profileService_1.isUserProfileComplete)(testCompletedUser), 'Database updated user with required fields is evaluated as complete');
        // 6. Test Error Response Format
        console.log('\n── 6. Testing Standardized Incomplete Profile Error Response ──');
        assert(profileService_1.PROFILE_INCOMPLETE_RESPONSE.code === 'PROFILE_INCOMPLETE', 'Error code matches PROFILE_INCOMPLETE');
        assert(profileService_1.PROFILE_INCOMPLETE_RESPONSE.success === false, 'Error response success is false');
        assert(profileService_1.PROFILE_INCOMPLETE_RESPONSE.message.includes('complete your profile'), 'Error message prompts profile completion');
        assert(profileService_1.BEEKEEPER_PROFILE_INCOMPLETE_RESPONSE.code === 'PROFILE_INCOMPLETE', 'Beekeeper error code matches PROFILE_INCOMPLETE');
        assert(profileService_1.BEEKEEPER_PROFILE_INCOMPLETE_RESPONSE.message === 'Please complete your beekeeper profile before adding a hive.', 'Beekeeper error message matches exact requirement: Please complete your beekeeper profile before adding a hive.');
        ...
        const beekeeperIdA = yield (0, authRoutes_1.generateUniqueBeekeeperId)();
        const beekeeperA = yield prisma.user.create({
            data: {
        ...
        const beekeeperIdB = yield (0, authRoutes_1.generateUniqueBeekeeperId)();
        const beekeeperB = yield prisma.user.create({
            data: {
                name: 'Beekeeper Bob',
        ...
        const hiveA1 = yield prisma.hive.create({
            data: {
        ...
        const hiveA2 = yield prisma.hive.create({
            data: {
                userId: beekeeperA.id,
                name: 'Meadow Beta',
                hiveCode: codeA2,
        ...
        const hiveB1 = yield prisma.hive.create({
            data: {
        ...
        // Query isolation
        const aliceHives = yield prisma.hive.findMany({ where: { userId: beekeeperA.id } });
        const bobHives = yield prisma.hive.findMany({ where: { userId: beekeeperB.id } });
