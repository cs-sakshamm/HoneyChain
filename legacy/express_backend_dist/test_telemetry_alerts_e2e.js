...
function runE2ETelemetryAlertTests() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield prisma.hiveAlert.deleteMany({ where: { hiveId: hive.id } });
            yield prisma.hiveTelemetry.deleteMany({ where: { hiveId: hive.id } });
            console.log('--- 1. Ingesting Normal Baseline Telemetry (32.4°C, 62% humidity, 24.5 kg weight) ---');
            yield prisma.hiveTelemetry.create({
                data: {
                    hiveId: hive.id,
            ...
            const anomalyResult = yield anomalyService_1.anomalyService.evaluateTelemetry(incomingPayload);
            ...
            const createdAlert = yield prisma.hiveAlert.create({
                data: {
                    hiveId: hive.id,
                    hiveCode: hive.hiveCode,
            ...
            const acknowledgedAlert = yield prisma.hiveAlert.update({
